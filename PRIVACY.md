...
- **Search and content requests:** Quran selections, translation or recitation choices, and Quran search terms needed for an opened Quran feature are sent to AlQuran.cloud. For supported English topic searches, a small set of related translation terms may also be sent. Nearby-mosque coordinates are sent to OpenStreetMap Overpass. These requests are not tied to a Duavara account because no account exists.
...
- **AlAdhan:** prayer-time, Qibla, Hijri, and related requests use the selected or device-provided latitude and longitude, plus the requested date and calculation settings. When Automatic calculation is selected, the coordinates let AlAdhan choose its closest supported authority.
...
- **AlQuran.cloud:** Quran requests use the surah, translation, recitation, or search selection needed to provide the requested Quran content. A supported English topic search can also send a small set of related translation terms.
